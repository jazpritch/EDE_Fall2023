...
\center Table 1: EPA Air Quality \center 
...
```{r Creating Boxplots, echo=FALSE, fig.align='left', fig.cap="Montlhy Ozone, 2018", fig.height=3, warning=FALSE}

Ozone_2018 <- epa_data %>% filter(Year == 2018) %>% ggplot(aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone)) + scale_x_discrete(name="Month", drop =FALSE) + geom_boxplot()+ labs(title = "Recorded Ozone by Month (2018)",
x = "Month", y = "expression(Concentration (mu*g / L)")
print(Ozone_2018)
```
```{r Creating Boxplots 2, echo=FALSE, fig.align='left', fig.cap="Montlhy Ozone, 2019", warning=FALSE, fig.height=3}

Ozone_2019 <- epa_data %>% filter(Year == 2019) %>% ggplot(aes(x = factor(Month, levels = 1:12, labels = month.abb), y = Ozone)) + scale_x_discrete(name="Month", drop =FALSE) + geom_boxplot()+ labs(title = "Recorded Ozone by Month (2019)",
    x = "Month", y = "expression(Concentration (mu*g / L)")
print(Ozone_2019)
